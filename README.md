...
本地启动后访问 <http://localhost:8000/ui>。以下截图对应 `docs/` 目录中的 PNG 文件。

> **GitHub 上不显示图片？** README 使用相对路径引用 `docs/*.png`。若远程仓库里没有这些文件（例如从未执行 `git add docs/` 并推送），页面会显示裂图。请把 `docs/` 目录提交进仓库后再 push。
